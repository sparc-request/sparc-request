...
RSpec.describe "Audit Reporting", js: true do
  ...
  let!(:super_user)          { create(:super_user, organization_id: program.id, identity_id: jug2.id)}#changed to lane here be careful to ensure this doesn't break anything else
  ...
  let!(:subsidy)             { create(:subsidy, pi_contribution: 2500, sub_service_request_id: sub_service_request.id)}
