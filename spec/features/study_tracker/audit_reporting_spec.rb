...
describe "generating an audit report" do 
    ...
    it "should pick an end date" do
      wait_for_javascript_to_finish
      find(:xpath, "//input[@id='cwf_audit_end_date']").click
      page.execute_script %Q{ $('#protocol_end_date_picker:visible').focus() }
      page.execute_script %Q{ $("a.ui-state-default:contains('1')").trigger("click") } # click on day 1 in the current month
      page.find('#cwf_audit_end_date_input').should_not have_value nil
    end 
    it "should pick different services" do 
      wait_for_javascript_to_finish
      page.uncheck('Nursing')
      page.uncheck('PFT Services')
      page.has_checked_field?('Nursing').should eq(false)
    end 
