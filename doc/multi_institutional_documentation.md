...
###Contents
	...
	3 Deploy Settings
		3.1 deploy.rb
		3.2 Deploy Environments
	4 HTML Content
		4.1 Header Contents
		4.2 Page Contents and Attribute Labels
	5 Data Flags
		5.1 CRTC Institutions
		5.2 Catalog Overlords

###Database Settings
####1.1 Adapters
SPARC has been designed to use the mysql2 adapter, however fairly extensive testing has also been done with the sqlite3 adapter.  No postgresql adapters have been tested with the system, however they may well work.  If you desire to use any adapter other than mysql2 *heavy* hesting is advised, starting with the test suite to attempt to probe for issues.
####1.2 Environments
SPARC Rails environment configurations are mostly standard.  However the following method:

	config.action_mailer.default_url_options
will need to be set in each environment accodring to the sending url. Also in the development environment the letter_opener gem is utilized and thus the option:

	config.action_mailer.delivery_method = :letter_opener
should be set.

In the production environment ExceptionNotifier is setup, and its options will need to be set to the individuals that you would like to send and recieve notifications when exceptions occur in production, i.e.:
...
####2.1 application.yml
Several important settings can be found in config/application.yml.  There is an example file found at config/application.yml.example in the repo which shows available settings with example imputs.  They are explained as follows:
...
- user_portal_link: This setting will soon be depricated.  Sets the location of the user portal.
...
- Associated User Attributes
	...
	- institutions: This is a list of institutions which a given identity can be associated with.  Include whatever instutions you would like your users to be able to associate themselves with. (Examples: University of South Carolina, Clemson University)
...
- funding_statuses: There are several imporant pieces of logic in the application that depend on whether a Project/Study is 'Funded' or 'Pending Funding'.  If you would like to add more options to the list, or to change the current options, you will need to modify the application logic which is defined by these options.
...
####2.3 ldap.yml
Settings relevant to your institution's LDAP can be found in config/ldap.yml. There is an example file found at config/ldap.yml.example in the repo which shows available settings with example imputs.  They are explained as follows:
...
####2.4 navigation.yml
This file lays out the navigation instructions for the service request portion of SPARC. Aside from chainging the 'step_text' or the 'css_class' of steps, the contents of this file should not be edited unless you have made significant changes to the application.  Each 'step' has certain paramters:
...
The file formats are not important, howeve the sizes which are shown in the filenames (in pixels), as well as the filenames themselves, should be maintained.  If you wish to use images that do not fit those size specifications you will need to do more work in the stylesheets to insure that the header is still in allignment.
...
###Data Flags
####5.1 CTRC Institutions
Any organization (Institution/Provider/Program/Core) that is a CTRC organization can have the attribute 'is_ctrc' set to true to get a set of features activated in the application which are unique to CTRC applications, for instance, the 'CTRC Approved' status for service requests.
