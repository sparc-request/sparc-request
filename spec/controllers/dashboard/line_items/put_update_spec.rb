  ...
  describe "PUT #update" do
    context "params[:line_item] is a valid update to LineItem" do
      before(:each) do
        ...
        allow(@line_item).to receive_message_chain(:service, :one_time_fee).and_return("am i a one time fee?")
      ...
      it "should update attributes of LineItem from params[:id]" do
        expect(@line_item).to have_received(:update_attributes).with("line item attributes")
      end
    ...
    context "params[:line_item] is not a valid update to a LineItem" do
      before(:each) do
        @line_item = findable_stub(LineItem) do
          instance_double(LineItem, id: 1, errors: "my errors")
        end
        allow(@line_item).to receive_message_chain(:service, :one_time_fee).and_return("am i a one time fee?")
